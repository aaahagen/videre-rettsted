...
- **Automatic Route Distance Calculation:** The app now automatically calculates the total distance of a route based on the geographical coordinates of the places it contains. The distance is displayed on the main routes page and updates in real-time as places are added or removed in the route editor.
- **Data Import Utility**: Added a new data import feature for administrators. Admins can now upload the JSON and ZIP files generated by the "Data Export" tool to completely restore or migrate places (including all text data, coordinates, and images) into a new organization.
...
### Changed
- **Overhauled Route Management:** The entire route creation and management feature has been redesigned for a more logical and intuitive user experience. The new workflow is: create a named route, add places to it, and the system handles the rest.

...
- **Intelligent Fleet & Physical Constraints:** Upgraded the `ConstraintEngine` and `VehicleForm` to reflect complex logistical realities.
    ...
    - **Vehicle Range Monitoring:** Added `maxRange` to vehicles. The routing engine ensures calculated routes do not exceed the fuel/charge capacity of electric or gas vehicles.
    - **Site-Specific Physical Constraints:** Places now support limits for `maxVehicleHeight`, `maxVehicleWidth`, `maxVehicleLength`, and `maxVehicleWeight`. The routing engine performs a "Physical Firewall" check to prevent vehicles from being assigned to destinations they cannot physically reach (e.g., due to low bridges or narrow gates).
- **Constraint-Based Routing Engine (Foundation):** Created `src/lib/routing-engine.ts` establishing the core logic for intelligent route automation. 
    - **Vehicle Constraints:** The engine actively checks if orders exceed a vehicle's maximum weight or volume (with configurable safety buffers, e.g., 85%).
    - **Capability Constraints:** The engine validates specific order requirements against vehicle capabilities (e.g., preventing ADR goods from being assigned to non-ADR vehicles).
    - **Time Window Validation:** The engine calculates estimated arrival times and compares them against the destination's "Leveringsvindu" (Delivery Window), generating warnings if the ETA falls outside open hours.
    - **Driver Work Hours Integration:** The engine cross-references the estimated route duration against the assigned driver's standard working hours. It generates soft warnings for potential overtime and hard errors if the route exceeds maximum legal daily driving limits (e.g., EU 561/2006 regulations).
    - **Cyborg Planning Approach:** Designed specifically to output "Soft Warnings" rather than hard crashes, allowing the human route planner to review the AI's suggestions and manually override or accept calculated risks based on real-world knowledge.
- **Collapsible Opening Hours UI:** Redesigned the "Åpningstider" section in the Place Form into a focused collapsible card. This keeps the form compact while providing easy access to weekly scheduling.
- **Schedule Copy Helper:** Added a "Bruk mandag på alle dager" shortcut to the opening hours registration, allowing for rapid duplication of schedules across the entire week.
- **Intelligent Route ETA & Opening Hours Validation:** Integrated time-window constraints into the route planning engine. 
    - **Route Start Time:** Admins can now define the exact departure time for a route.
    - **Dynamic ETA Tracking:** The system calculates a per-stop ETA based on accumulated travel time, service durations, and prep times.
    - **Opening Hours Enforcement:** Every stop is validated against the destination's weekly schedule. A high-visibility warning badge and red styling alert planners if a delivery is scheduled outside of a location's operating hours.
- **Place Opening Hours (Phase A):** Implemented a comprehensive scheduling system for delivery locations. Administrators can now define daily opening and closing times for each place. This data is visualized in a dedicated grid on the place details page.
- **LMS Storage Integration:** Upgraded the Learning Management System to support direct file uploads. Administrators can now upload PDF and Video modules directly to Firebase Storage during course creation, with automatic organization and real-time upload progress feedback.
...
- **Learning Management System (LMS) Foundation:** Launched a comprehensive training module.
    ...
    - **Admin Library:** A management console for administrators to create, edit, and bulk-assign courses to the organization.
    - **Progress Tracking:** Real-time tracking of course completion status and individual lesson progress.
- **Bulk Order Import:** Implemented a new CSV-based bulk import tool in the Admin Console. Features auto-column mapping, a validation preview table, and real-time import progress tracking for high-volume logistical operations.
- **Attendance Statistics Card:** Added a new real-time tracking card to the Admin Dashboard. It displays high-level metrics for today's attendance, including personnel currently present, those who have finished their shifts, and those still waiting to clock in, cross-referenced with the daily schedule.
- **Rapid "Scan-to-Receive" Workflow:** Implemented a high-speed mobile ingestion interface for terminal workers at `/dashboard/loader/scan-to-receive`. This allows for rapid scanning of 3rd party packages, automatically generating "shell" orders and optionally assigning them to active routes in real-time.
...
- **Place Opening Hours (Complex Implementation):** Implement a system to define opening hours for each delivery/pickup location.
    - **Phase A (Data):** Extend the `Place` model to store a 7-day schedule (open/close times or marked closed). Add this to the "Create/Edit Place" form. (Completed)
    - **Phase B (Route Logic - High Complexity):** Upgrade the route planning interface to actively warn planners if a calculated ETA falls outside a location's opening hours. (Completed)

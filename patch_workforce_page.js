...
export default function WorkforcePage() {
    ...
    // Print state
    const [printDriver, setPrintDriver] = useState<DriverProfile | null>(null);
    ...
    // Generate the 12-week grid data for the selected driver
    let printWeeks: { startDate: Date; days: { date: Date; status: any }[] }[] = [];
    if (printDriver) {
        const startOfPlan = startOfWeek(searchDate, { weekStartsOn: 1 }); // Monday
        for (let w = 0; w < 12; w++) {
            ...
            for (let d = 0; d < 7; d++) {
                ...
                days.push({
                    date: currentDay,
                    status: getDriverStatus(printDriver, currentDay)
                });
            }
    ...
    return (
        <>
            ...
            <div className="print:hidden container mx-auto max-w-7xl px-4 py-8 space-y-6">
                                    ...
                                    return (
                                        <div key={driver.id} className="p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:bg-slate-50 transition-colors">
                                            ...
                                            <div className="flex flex-col sm:flex-row items-end sm:items-center gap-3">
                                                ...
                                                <Button variant="secondary" size="sm" onClick={() => setPrintDriver(driver)}>
                ...
                <Dialog open={!!printDriver} onOpenChange={(open) => !open && setPrintDriver(null)}>
                    <DialogContent className="max-w-4xl w-[95vw] max-h-[90vh] overflow-y-auto">
                        <DialogHeader>
                            <DialogTitle>Forhåndsvisning av Arbeidsplan</DialogTitle>
                        </DialogHeader>
                        
                        {printDriver && (
                            <div className="space-y-4 py-4">
                                <p className="text-muted-foreground">
                                    Dette er planen for <strong>{printDriver.name}</strong> for de neste 12 ukene, med start fra <strong>{format(startOfWeek(searchDate, { weekStartsOn: 1 }), 'dd.MM.yyyy')}</strong>.
                                </p>
                                ...
                                <div className="border rounded-lg overflow-hidden">
                                    <table className="w-full text-sm text-left">
                                        <thead className="bg-slate-50 text-slate-700">
                                            <tr>
                                                <th className="px-4 py-3 font-semibold border-b">Uke</th>
                                                <th className="px-4 py-3 font-semibold border-b">Mandag</th>
                                                <th className="px-4 py-3 font-semibold border-b">Tirsdag</th>
                                                <th className="px-4 py-3 font-semibold border-b">Onsdag</th>
                                                <th className="px-4 py-3 font-semibold border-b">Torsdag</th>
                                                <th className="px-4 py-3 font-semibold border-b">Fredag</th>
                                                <th className="px-4 py-3 font-semibold border-b bg-slate-100">Lørdag</th>
                                                <th className="px-4 py-3 font-semibold border-b bg-slate-100">Søndag</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y">
                                            {printWeeks.map((week, wIndex) => (
                                                <tr key={wIndex} className="hover:bg-slate-50">
                                                    <td className="px-4 py-3 font-medium border-r bg-slate-50">
                                                    ...
                                                    {week.days.map((day, dIndex) => (
                                                        <td key={dIndex} className={cn("px-4 py-3 border-r last:border-r-0", (dIndex === 5 || dIndex === 6) && "bg-slate-50/50")}>
                                                            <div className="font-medium text-slate-900">{day.status.short}</div>
                                                            <div className="text-[10px] text-muted-foreground">{format(day.date, 'dd.MM')}</div>
                                                        </td>
                                                    ))}
                                                </tr>
                                            ))}
                        ...
                        <DialogFooter>
                            <Button variant="outline" onClick={() => setPrintDriver(null)}>Avbryt</Button>
                            <Button onClick={handlePrint}><Printer className="mr-2 h-4 w-4" /> Skriv ut</Button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
            </div>
            ...
            {printDriver && (
                <div className="hidden print:block p-8 bg-white min-h-screen text-black">
                    <div className="mb-8 border-b pb-4">
                        <h1 className="text-3xl font-bold mb-2">Arbeidsplan: {printDriver.name}</h1>
                        <p className="text-gray-600">
                            Periode: {format(printWeeks[0].startDate, 'dd.MM.yyyy')} - {format(printWeeks[11].days[6].date, 'dd.MM.yyyy')}
                        </p>
                        <p className="text-gray-600">Utskrevet: {format(new Date(), 'dd.MM.yyyy HH:mm')}</p>
                    </div>

                    <table className="w-full text-sm text-left border-collapse border border-gray-300">
                        <thead className="bg-gray-100 text-gray-800">
                            <tr>
                                <th className="px-3 py-2 border border-gray-300">Uke</th>
                                <th className="px-3 py-2 border border-gray-300">Mandag</th>
                                <th className="px-3 py-2 border border-gray-300">Tirsdag</th>
                                <th className="px-3 py-2 border border-gray-300">Onsdag</th>
                                <th className="px-3 py-2 border border-gray-300">Torsdag</th>
                                <th className="px-3 py-2 border border-gray-300">Fredag</th>
                                <th className="px-3 py-2 border border-gray-300 bg-gray-50">Lørdag</th>
                                <th className="px-3 py-2 border border-gray-300 bg-gray-50">Søndag</th>
                            </tr>
                        </thead>
                        <tbody>
                            {printWeeks.map((week, wIndex) => (
                                <tr key={wIndex}>
                                    <td className="px-3 py-2 border border-gray-300 bg-gray-50 font-medium text-center">
                                        {format(week.startDate, 'ww', { locale: nb })}
                                    </td>
                                    {week.days.map((day, dIndex) => (
                                        <td key={dIndex} className={cn("px-3 py-2 border border-gray-300", day.status.type === 'off' ? 'text-gray-500' : 'font-medium', (dIndex === 5 || dIndex === 6) && 'bg-gray-50')}>
                                            <div>{day.status.short}</div>
                                            <div className="text-[10px] text-gray-400 mt-1">{format(day.date, 'dd.MM')}</div>
                                        </td>
                                    ))}
                                </tr>
                            ))}
                    ...
                    <div className="mt-8 text-xs text-gray-500">
                        * Planen kan endres ved sykdom, ferie eller andre avvik. Generert fra VIDERE RettSted.
                    </div>
                </div>
            )}
        </>
...
console.log('Added 12-week print functionality to WorkforcePage');
